...
%Note: The code is edited version of original BBO provided by Prof Dan
%Simon

function [MinCost, Hamming] = SBBO1(ProblemFunction, DisplayFlag, ProbFlag, RandSeed)
...
OPTIONS.pmutate = 0.1; % initial mutation probability
...
for GenIndex = 1 : OPTIONS.Maxgen
    ...
    if ProbFlag
        ...
        for k = round(length(Population)/2) : length(Population)
            ...
            a2=-1+GenIndex*((-1)/OPTIONS.Maxgen);
            ...
            if MutationRate(k) > rand
                for parnum = 1 : OPTIONS.numVar
                    if p<0.5
                        ...
                        rand_leader_index = floor(length(Population)*rand()+1);
                        X_rand = Population(rand_leader_index).chrom;
                        D_X_rand=abs(C*X_rand(parnum)- Population(k).chrom(parnum)); 
                        Island(k,parnum)=X_rand(parnum)-D_X_rand;     
                        
                    else
                        
                        distance2Leader=abs(Population(1).chrom(parnum)-Population(k).chrom(parnum));
                       
                        Island(k,parnum)=distance2Leader*exp(b.*l).*cos(l.*2*pi)+Population(1).chrom(parnum);
